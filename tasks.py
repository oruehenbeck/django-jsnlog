...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-jsnlog.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ jsnlog")
...
def lint(c):
    ...
    """
    c.run("flake8 django-jsnlog tests")
...
def release(c, bumpsize=''):
    ...
    import django_jsnlog
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_jsnlog.__version__))
    c.run("git push --tags")
